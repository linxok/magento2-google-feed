...
5. [Призначення категорій для продуктів](#5-призначення-категорій-для-продуктів)
6. [Робота з фідом](#6-робота-з-фідом)
7. [Налаштування мультимагазину](#7-налаштування-мультимагазину)
8. [Автоматична генерація фіду (Cron)](#8-автоматична-генерація-фіду-cron)
9. [Маппінг атрибутів продуктів](#9-маппінг-атрибутів-продуктів)
10. [Вирішення проблем](#10-вирішення-проблем)
...
Модуль автоматично визначає, яку Google Product Category використовувати для кожного продукту за наступним пріоритетом (перший знайдений виграє):

```
1. Власна Google Product Category атрибут продукту
        ↓ (якщо не встановлено)
2. Google Product Category прямої категорії продукту
        ↓ (якщо не встановлено)
3. Google Product Category батьківської категорії
        ↓ (якщо не встановлено — піднімається по дереву вище)
4. Категорія діда → ... → Коренева категорія
        ↓ (якщо нічого не знайдено)
5. Поле не включається у фід
```
...
Електроніка [Google Category: 222 — Electronics]
  └─ Телефони [Google Category: 267 — Mobile Phones]
      └─ Смартфони [Не встановлено → успадковує 267]
          ...
          └─ Продукт B [Google Category: 268 — Smartphones → використовує власне]
```
...
## 5. Призначення категорій для продуктів

Ви можете призначити Google Product Category безпосередньо окремому продукту. Це перевизначає будь-яке призначення на рівні категорії.

### Покрокова інструкція

1. Перейдіть до **Catalog → Products**
2. Відкрийте будь-який продукт для редагування
3. Перейдіть на вкладку **General** або **Search Engine Optimization** (залежно від версії Magento та конфігурації)
4. Знайдіть атрибут **Google Product Category**
5. Натисніть **Select Category** і скористайтесь вибірником (так само, як у категоріях)
6. Натисніть **Save** для збереження продукту

> **Коли використовувати призначення на рівні продукту:**
> - Продукт належить до категорії з загальним призначенням, але потребує більш конкретного
> - Продукт не вписується в Google-категорію своєї категорії
> - Ви хочете перевизначити успадкування для конкретного продукту
...
## 8. Автоматична генерація фіду (Cron)
...
### Google Product Category не відображається у формі категорії/продукту
```bash
php bin/magento cache:flush
php bin/magento mycompany:googlefeed:import-taxonomy
```
...
### Збережена Google Product Category не відображається при повторному відкритті форми
- У поточній версії це вирішено — назва автоматично підтягується при завантаженні форми через AJAX
- Якщо проблема зберігається, очистіть кеш: `php bin/magento cache:flush`
