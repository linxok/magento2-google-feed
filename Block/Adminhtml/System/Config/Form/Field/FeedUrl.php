...
class FeedUrl extends Field
{
    ...
    protected function _getElementHtml(AbstractElement $element)
    {
        ...
        foreach ($websites as $website) {
            ...
            if (count($stores) > 0) {
                ...
                $html .= '<h4 style="margin: 0 0 10px 0; color: #333;">'
                    . $this->escapeHtml($website->getName()) . '</h4>';

                foreach ($stores as $store) {
                    $storeCode = $store->getCode();
                    $storeName = $store->getName();
                    $feedUrl = $this->storeUrlResolver->getFeedUrl($store);

                    $uniqueId = 'googlefeed_url_' . $store->getId();
                    $messageId = 'copy_message_' . $store->getId();

                    $html .= '<div style="margin-bottom: 15px; padding: 10px; background: #fff;'
                    ...
                    $html .= '<span style="padding: 2px 8px; background: #e3f2fd; color: #1976d2;'
                    ...
                    $html .= 'id="' . $uniqueId . '" ';
                    $html .= 'style="flex: 1; padding: 6px 10px; border: 1px solid #ccc; border-radius: 3px;'
                        . ' background: #fafafa; font-family: monospace; font-size: 12px;" />';
                    $html .= '<button type="button" class="action-default scalable"'
                        . ' onclick="copyUrl(\'' . $uniqueId . '\', \'' . $messageId . '\')"'
                        . ' style="padding: 6px 12px;">';
                    $html .= '<span>' . __('Copy') . '</span>';
                    $html .= '</button>';
                    $html .= '<a href="' . $this->escapeUrl($feedUrl) . '" target="_blank"'
                        . ' class="action-default scalable" style="padding: 6px 12px; text-decoration: none;">';
                    $html .= '<span>' . __('Open') . '</span>';
                    $html .= '</a>';
                    $html .= '</div>';

                    // Copy success message
                    $html .= '<div id="' . $messageId . '" style="margin-top: 6px; color: #2e7d32;'
        ...
        $html .= '<div style="margin-top: 15px; padding: 10px; background: #e3f2fd;'
            . ' border-left: 4px solid #1976d2; border-radius: 3px;">';
